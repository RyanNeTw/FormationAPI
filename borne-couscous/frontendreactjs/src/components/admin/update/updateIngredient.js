...
export default function UpgradeIngredient({ ingredient, setIngredient }){
    ...
    const onSubmit = data => {updateIngredient(data), console.log(data)};

    const [dataUpdate, setDataUpdate] = useState("");

    function updateIngredient(data){
        fetch('http://127.0.0.1:3333/ingredients/updateIngredient/' + location.state.id, {
            method: 'PUT',
            headers:{
    ...
    return(
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2">
                ...
                <input placeholder="stock" type="number" defaultValue={location.state.stock} {...register("stock")} className="border-solid border-sky-500 border-2 text-sky-500" />
