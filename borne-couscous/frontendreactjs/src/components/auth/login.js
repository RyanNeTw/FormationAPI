...
export default function Login() {
    ...
    const {setTokenStore, setUserStore, setRoleStore} = useContext(StoreContext);
    ...
    function getUser(data){
      fetch('http://127.0.0.1:3333/auth/me', {
        method: 'GET',
        headers:{
        'Content-Type':'application/json',
        ...
        res.json().then((json) =>{
          getRole(json, data.token.token)
          setUserStore(json.auth.guards.api.user)
          localStorage.setItem('user', json.auth.guards.api.user)
          console.log(json)
        })
    })
  }

  function getRole(data, token){
        fetch('http://127.0.0.1:3333/role/' + data.auth.guards.api.user.role_id , {
          method: 'GET',
          headers:{
          'Content-Type':'application/json',
    ...
    const onSubmit = data => {loginApi(data), setShowModal(false)};
    return (
        <>
          <button
